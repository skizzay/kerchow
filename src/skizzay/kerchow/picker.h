...
template <class ForwardIterator,
          class = typename std::enable_if<
              !std::is_arithmetic<ForwardIterator>::value>::type>
struct enable_forward_iterator
    : std::enable_if<is_forward_iterator<ForwardIterator>::value,
                     ForwardIterator> {};

template <class ArithmeticType>
struct enable_arithmetic
    : std::enable_if<std::is_arithmetic<ArithmeticType>::value,
                     ArithmeticType> {};
}
...
public:
   ...
   inline void seed(result_type seed) { generator.seed(seed); }
   ...
   inline void discard(unsigned long long const z) { generator.discard(z); }

   template <class ArithmeticType1, class ArithmeticType2 = ArithmeticType1>
   typename details_::enable_arithmetic<
       typename std::common_type<ArithmeticType1, ArithmeticType2>::type>::type
   pick(ArithmeticType1 const low = std::numeric_limits<ArithmeticType1>::min(),
        ArithmeticType2 const high = std::numeric_limits<ArithmeticType2>::max()) {
      using t = typename std::common_type<ArithmeticType1, ArithmeticType2>::type;
      assert((t(low) < t(high)) && "Low must strictly be less than high.");

      typename details_::uniform_distribution<t>::type distribution{t(low),
                                                                    t(high)};
      return distribution(generator);
   }

   template <class ForwardIterator>
   typename details_::enable_forward_iterator<ForwardIterator>::type pick(
       ForwardIterator const begin, ForwardIterator const end) {
      using std::distance;
      using std::next;
   ...
   auto pick(const Container &container) ->
       typename details_::enable_forward_iterator<
   ...
   fuzzy_number<ArithmeticType> create_fuzzy_number(
      ...
      ArithmeticType const a = pick(low, high);
      ArithmeticType const b = [&]() noexcept {
         ArithmeticType tmp = pick(low, high);
         while (tmp == a) {
            tmp = pick(low, high);
         }
         return tmp;
      ...
      return fuzzy_number<ArithmeticType>{generator, std::min(a, b),
                                          std::max(a, b)};
   }

   template <class T, class... ValueArgs>
   fuzzy_container<T, generator_type> create_fuzzy_container(
...
class basic_random_picker<std::random_device> {
  public:
   ...
   inline basic_random_picker(const std::string &token) : generator(token) {}
   ...
   inline double entropy() const noexcept { return generator.entropy(); }

   template <class ArithmeticType>
   typename details_::enable_arithmetic<ArithmeticType>::type pick(
       ArithmeticType low = std::numeric_limits<ArithmeticType>::min(),
       ArithmeticType high = std::numeric_limits<ArithmeticType>::max()) {
      assert((low < high) && "Low must strictly be less than high.");

      typename details_::uniform_distribution<ArithmeticType>::type
   ...
   fuzzy_number<ArithmeticType> create_fuzzy_number(
      ...
      return fuzzy_number<ArithmeticType>{pick<fuzzy_seed_type>(),
